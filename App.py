...
class App(ttk.Frame):
    def __init__(self, parent):
        ...
        radBtn = ttk.Radiobutton(self.frameTemas,text="Oscuro",value="awdark",padding=5,variable=self.nombreTema,command=self.setTheme)
    ...
    def cargarComponentes(self):
        ...
        ttk.Radiobutton(self.frameTemas,text="Claro",value="awlight",padding=5,variable=self.nombreTema,command=self.setTheme).grid(column=0,row=1)
        self.frameTemas.grid(column=0,row=1,pady=5)
        self.__lblReferencia = ttk.LabelFrame(self,text='Niveles de importancia',padding=5,borderwidth=5,relief='solid')
        ttk.Label(self.__lblReferencia,text='IMPORTANTE',font='Helvetica 8 bold',padding=(15,5,15,5),background='red',foreground='white',borderwidth=5,relief='solid',width=12).grid(column=0,row=0,pady=5,padx=5)
        ttk.Label(self.__lblReferencia,text='NORMAL',font='Helvetica 8 bold',padding=(27,5,0,5),background=self.configTema['bgLabelNormal'],borderwidth=5,relief='solid',width=12).grid(column=0,row=1,pady=5,padx=5)
    ...
    def __cargarMenuLateral(self):
        ...
        self.btnModif = ttk.Button(self.__menuFrame,text="Modificar Evento",style='Botones.TButton',width=15,command=self.__modificar,padding=5,state='disabled')
        ...
        self.__btnNuevo = ttk.Button(self.__menuFrame,text="Nuevo Evento",command=self.__nuevoEvento,padding=5,width=15,)
        ...
        self.btnElim = ttk.Button(self.__menuFrame,text="Eliminar Evento",width=15,command=self.__eliminar,padding=5,state='disabled')
    ...
    def setTheme(self):
        ...
        if self.__primerInicio:
            ...
            s.map('btnAceptar.TButton',background=[('!active','#0047cc'),('pressed', '#03009d'), ('active', '#3b7fff')])
            s.map('btnCancelar.TButton',background=[('!active','#d40000'),('pressed', '#d46300'), ('active', '#ff3d45')])
            s.map('btnSigAnt.TButton',background=[('!active','#6a20a2'),('pressed', '#4a1572'), ('active', '#a046e5')])
            ...
            if opt == True:
                ...
                if self.nombreTema.get() == 'awdark':
                    ...
                    s.map('btnAceptar.TButton',background=[('!active','#0ed145'),('pressed', '#1f9d43'), ('active', '#2cfe67')])
                    s.map('btnCancelar.TButton',background=[('!active','#ff0000'),('pressed', '#b40000'), ('active', '#ff5252')])
                    s.map('btnSigAnt.TButton',background=[('!active','#53a9f4'),('pressed', '#0082f4'), ('active', '#88c2f5')])
