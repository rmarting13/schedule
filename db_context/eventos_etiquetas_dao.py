...
class EventoEtiquetaDao:
    ...
    _INSERTAR = 'INSERT INTO eventos_etiquetas(id_evento, id_etiqueta) VALUES(%s, %s);'
    ...
    def seleccionar(cls, id_evento):
        with Cursor() as cursor:
            values = (id_evento, )
            cursor.execute(cls._SELECCIONAR, values)
    ...
    def eliminar(cls, **kwargs):
        with Cursor() as cursor:
            if kwargs.get('id_evento_etiqueta'):
                cursor.execute(cls._ELIMINAR_ID, kwargs.get('id_evento_etiqueta'))
            elif kwargs.get('id_evento') and kwargs.get('id_etiqueta'):
                print('ENTRO A LA ELIMINACION DE LA DB')
                values = (kwargs.get('id_evento'), kwargs.get('id_etiqueta'))
